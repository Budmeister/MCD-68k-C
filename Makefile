...
# LINKER_SCRIPT = D:/GccAshWare/gcc-m68k-ashware/m68k-coff/lib/m68000/mvme135.ld
# CFLAGS = -m68000 -O0 -T$(LINKER_SCRIPT) --verbose -Wl,--verbose -ansi -Wl,-Map=$(basename $@).map
...
INCLUDES = bin/easy68k-utils.o bin/malloc.o bin/string.o bin/io.o bin/math.o

test.o: test.c $(LINKER_SCRIPT)
	$(CC) $(CFLAGS) test.c -c -o test.o

# test.S: test.c $(LINKER_SCRIPT)
# 	$(CC) $(CFLAGS) test.c -S -o test.S

%.S: %.c $(LINKER_SCRIPT)
	$(CC) $(CFLAGS) $(basename $@).c -S -o $(basename $@).S
